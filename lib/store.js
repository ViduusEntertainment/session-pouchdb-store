...
const cookie_adapter = (target, source) => {
	return (source && source.toJSON !== undefined) ? source.toJSON() : source;
};
...
class SessionStore extends Store {
	...
	constructor(pouchDB,options) {
		...
		this.sessions = {};
		this.options = _.assignIn({}, DEF_OPTS, options);
		this._subscribe();
		this._timers();
	}
			...
			await Promise.all(
				Object.keys(self.sessions).
					...
					map(async(sid)=>{
						...
						sess.$dirty = false;
						let res = await self.pouch.put(_.assignInWith({}, sess, cookie_adapter));
						_.assignIn(sess, {_rev:res.rev});
					})
			);
	...
	_subscribe() {
		this.pouch.changes({
		  ...
		  include_docs: true
			...
			if(old) _.assignIn(old,change.doc);
		}).on('complete', info => {
			debug("====> COMPLETE",info);
		}).on('error', err => {
			debug("====> ERROR",err);
	...
	async get(sid, callback) {
		...
		if(this.sessions[sid]) {
			...
			try {
				callback(null, this.sessions[sid] = await this.pouch.get(sid,{attachments:true}));
			}catch(err) {
	...
	set(sid, session, callback) {
		...
		if(this.sessions[sid]) {
			...
			_.assignIn(this.sessions[sid],session,{_rev});
		}
		else {
		...
		if(session.$commit) {
			session.$dirty = false;
			this.pouch.put(_.assignInWith({}, this.sessions[sid], cookie_adapter),callback);
		}
		else {
			callback();
		}
	...
	touch(sid, session, callback) {
		...
		if(oldsession) {
			...
			_.assignIn(oldsession,session,{_rev});
